...
void insert_last(LIST &l, NDOE *p)
{
...
LIST daThuc(LIST &p, LIST &q)
{
    LIST r;
    init(r);
    NODE *m = p.pHead, *n = p.pHead;
    while (p.pHead == NULL && q.pHead == NULL)
    {
        if (m == NULL)
        {
            insert_daThuc_last(p, m->heSo, m->soMu);
            m = m->pNext;
        }else if(n==NULL){
            insert_daThuc_last(q, n->heSo, n->soMu);
            n=n->pNext;
